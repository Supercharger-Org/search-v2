...
class DisplayManager {
  ...
  updateDisplay() {
    ...
    this.steps.forEach((step) => {
      ...
      const selectedValue =
        step === "library" ? session.library : session.method.selected;
      if (selectedValue) {
        document
          .querySelectorAll(`[data-${step}-display]`)
          .forEach((element) => {
            const allowedValues = element.dataset[`${step}Display`]
              .split(",")
              .map((v) => v.trim());
  ...
  updateFilterDisplay(session) {
    ...
    const optionsWrapper = document
      .querySelector('[step-name="options"]')
  ...
  initializeClickEvents() {
    ...
    document.querySelectorAll("[data-library-option]").forEach((element) => {
      element.addEventListener("click", (e) => {
        const library = e.target.closest("[data-library-option]").dataset
          .libraryOption;
    ...
    document.querySelectorAll("[data-method-option]").forEach((element) => {
      element.addEventListener("click", (e) => {
        const method = e.target.closest("[data-method-option]").dataset
          .methodOption;
...
(() => {
  ...
  // Explicitly expose the app instance to the window object
  window.app = searchApp;

  // Log that the app is ready
  console.log("Search app initialized and exposed as window.app");
})();
